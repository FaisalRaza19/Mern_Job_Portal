      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <DashboardCard title="Quick Actions">
          <div className="space-y-3">
            ...
            <button onClick={() => setActiveTab("applicants")} className="w-full flex items-center space-x-3 p-3 bg-green-50 text-green-700 rounded-lg hover:bg-green-100 transition-colors">
        ...
        <div className="space-y-3">
          {recentApplications?.length < 0 ?
            ...
            recentApplications.map((application, index) => (
              <div onClick={() => setActiveTab("applicants")}
                key={index}
                className="flex items-center justify-between p-3 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors"
              >
                <div className="flex items-center space-x-3">
                  <img
                    src={application?.User?.avatar?.avatar_Url || "/placeholder.svg?height=40&width=40"}
                    alt={application?.User?.jobSeekerInfo?.fullName || ""}
                    className="w-10 h-10 rounded-full"
                  />
                  <div>
                    <p className="font-medium text-gray-900 ">{application?.User?.jobSeekerInfo?.fullName}</p>
                    <p className="text-sm text-gray-600 ">{application?.title || ""}</p>
                  </div>
                </div>
                <div className="text-right">
                  <span
                    className={`px-2 py-1 text-xs rounded-full ${application.status === "New"
                  ...
                  <p className="text-xs text-gray-500  mt-1">{new Date(application?.appliedAt).toLocaleDateString() || "null"}</p>
                </div>
              </div>
            ))}
        </div>
