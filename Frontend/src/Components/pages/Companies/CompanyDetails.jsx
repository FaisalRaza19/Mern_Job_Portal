    ...
    if (loading) {
        return (
            <main className="min-h-screen p-4 md:p-8">
                <Link to="/companies">
                    <button
                        className="mb-6 inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-900 shadow-sm transition-colors hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2"
                    >
    ...
    if (!companyDetails) {
        return (
            <main className="flex min-h-screen flex-col items-center justify-center ">
                <FiAlertCircle className="mb-4 h-16 w-16 text-gray-500" />
                <p className="text-xl font-semibold text-gray-900">Company not found.</p>
                <Link to="/companies">
                    <button
                        className="mt-6 inline-flex items-center justify-center rounded-md bg-gray-900 px-4 py-2 text-sm font-medium text-white shadow-sm transition-colors hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2"
                    >
                        <FaArrowLeft className="mr-2 h-4 w-4" />
                        Back to Companies
                    </button>
                ...
                <section className="rounded-xl bg-white p-6 shadow-sm">
                    <div className="flex flex-col items-center md:flex-row md:items-start md:space-x-6">
                        ...
                        <div className="text-center md:text-left">
                            <h1 className="text-4xl font-bold text-gray-900">{CompanyDetails?.companyInfo?.companyName || "Null"}</h1>
                            <div className="mt-2 flex items-center justify-center space-x-4 text-gray-700 md:justify-start">
                                <span className="flex items-center text-lg">
                                    <FaBriefcase className="mr-2 h-5 w-5" />
                                    {CompanyDetails?.companyInfo?.companyType || "Null"}
                                </span>
                                <span className="flex items-center text-lg">
                                    <FaMapPin className="mr-2 h-5 w-5" />
                                    {CompanyDetails?.companyInfo?.companyLocation.split(",")[0] || "Null"}
                                </span>
                            </div>
                            <div className="mt-4 grid grid-cols-1 gap-2 text-gray-700 md:grid-cols-2 md:gap-4">
                                <div className="flex items-center">
                                    <FaUsers className="mr-2 h-5 w-5 text-gray-500 " />
                                    <span>{CompanyDetails?.companyInfo?.companySize || 0}</span>
                                </div>
                                {CompanyDetails?.companyInfo?.companyWeb ? 
                                <a
                                ...
                                >
                                    ...
                                    {CompanyDetails?.email || "123@example.com"}
                                </a>
                            </div>
                        </div>
                    </div>
                    <div className="mt-6 border-t border-gray-200 pt-6">
                        <h2 className="mb-3 text-2xl font-bold text-gray-900">About Us</h2>
                        <p className="text-gray-700">{CompanyDetails?.companyInfo?.companyDescription || "Null"}</p>
                    </div>
                </section>
